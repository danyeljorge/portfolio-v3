...
export function TechIcon({ type, className = "w-9 h-9" }: { type: string; className?: string }) {
  switch (type) {
    ...
    case "nextjs":
      return (
        <div className="w-10 h-10 flex items-center justify-center font-display font-black text-2xl text-zinc-900 dark:text-white group-hover:text-emerald-600 dark:group-hover:text-brand-neon transition-colors">
          N
        </div>
      );
    case "typescript":
